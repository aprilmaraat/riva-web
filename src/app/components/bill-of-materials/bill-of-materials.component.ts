...
export class BillOfMaterialsComponent extends GenericComponent implements OnInit {
  ...
  //   console.log(this.tableB_filtered);
  //   console.log(this.sizeSelection);
  ...
  getOptionSizes(id: number){
    ...
    this.tableB.forEach(item => {
      info = this.sizeSelection.filter(x =>  x.parentId == id && x.id != item.subItemInfoId);
    });
    return info;
  }
